...
Brt_LUR = function(variabledf, opti = F, vis1 = T, ntree = 1000,   tree.complexity = 6,bag.fraction = 0.5,shrinkage= 0.01, y_varname = c("day_value", "night_value", "value_mean"), training, test, grepstring = "ROAD|pop|temp|wind|Rsp|OMI|eleva|coast", ...) {
    ...
    if (opti) {
        ...
        rf3 = gbm(formula = formu, data = pre_mat, distribution = "gaussian", n.trees = ntree,  tree.complexity = tree.complexity, shrinkage = shrinkage, bag.fraction = bag.fraction)
    }
