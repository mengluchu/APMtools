...
#' @export
#'
scatterplot = function(variabledf, y_name, fittingmethod = "lm", n_row = 5) {
    # quote(y_var)
    variabledf %>% gather(VAR, predictors, -y_name) %>% ggplot(aes_string(x = "predictors", y = y_name)) + geom_point() + facet_wrap(~VAR, scales = "free", nrow = n_row) + stat_smooth(method = fittingmethod) + theme_bw()
}
